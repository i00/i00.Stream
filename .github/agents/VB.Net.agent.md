...
- If the commit creates, updates, or otherwise relates to a plan document (e.g. under `.claude/plans`), use `[Plans]` in place of a project name, with one bullet per plan touched. A `[Plans]` section always comes first, before any project sections and before `[General]`. Also mention the plan by name in the bullet list of any project section its changes relate to, so the connection is visible from that section too.
...
- List every touched project as its own section (single line break), in the most logical order, each with its own bullet list. `[Plans]` comes first if present, followed by project sections, followed by `[General]` if present.
